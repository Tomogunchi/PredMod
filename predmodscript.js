...
function populatePaths(node) {
    ...
    for (let i = 0; i < pathparts.length; i++) {
        ...
        if (i == partpaths.length - 1) {
            target[part] = pc.app.root.findByPath(path);
        }
        else {
    ...
    node.children.forEach(populatePaths);
}

//Then, for example,
populatePaths(pc.app.root);
console.log(window.MenuHolder.Menu);
...
function loadGameScript() {
    ...
    const getPaths = (node) => {
        return [node.path, ...node.children.map(child => getPaths(child))]
    }
    console.log(getPaths(pc.app.root));
    // MENU
